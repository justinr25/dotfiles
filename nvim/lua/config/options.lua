...
vim.opt.conceallevel = 2 -- Hide markup/formatting markers (like markdown, JSON quotes) (Obsidian requirement)
vim.opt.concealcursor = "" -- Don't hide cursor line markup
vim.opt.lazyredraw = false -- Don't redraw during macros (disabled to fix search count)
...
vim.opt.updatetime = 300 -- Faster completion (time in ms to trigger CursorHold)
vim.opt.timeoutlen = 500 -- Time in ms to wait for mapped sequence
vim.opt.ttimeoutlen = 0 -- No wait for key code sequences (Key code timeout)
...
-- Create and set undo directory if it doesn't exist
local undodir_path = vim.fn.expand("~/.local/share/nvim/undodir") -- Get absolute path for undo directory
vim.opt.undodir = undodir_path -- Configure Neovim to use this directory

if vim.fn.isdirectory(undodir_path) == 0 then -- Check if the directory doesn't exist
	vim.fn.mkdir(undodir_path, "p") -- Create the folder (including parent directories)
end
...
-- Cursor Settings
vim.opt.guicursor = {
	"n-v-c-ve:block", -- Normal, Visual, Command-line, Visual-exclusive
...
vim.opt.foldexpr = "v:lua.vim.treesitter.foldexpr()" -- Use built-in treesitter for folding
